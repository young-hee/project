...
public class IniPayment {
       ...
       //공통 parameter 셋팅       	
	    ...
	    // 신용카드 결제
	   	...
	   	// 가상계좌
	   		...
	   		// TODO : 가상계좌 입금기한 날짜
	   		map.put("P_VBANK_DT", addHHDateString(24));
	   		// TODO : 가상계좌 입금기한 시간
	   	}
    ...
    public static HashMap<String, Object> makeRequestParam(PayDTO payDTO) throws Exception{ 
       ...
       // 가상계좌이체
	   		...
	   		//acceptmethod make
	   		String depositDate = addHHDateString(24);
	   		map.put("acceptmethod", "vbank(" + depositDate + ")");
            ...
            if("00".equals(paramMap.get("P_STATUS")) && !StringUtils.isEmpty(paramMap.get("P_TID"))){
                ...
                try {
                    ...
                    if (200 == resStatCode) { // HTTP Status-Code 200: OK.
                        ...
                        BufferedReader bReader = new BufferedReader(new InputStreamReader(inStream, "EUC-KR"));
                        String readLine = "";

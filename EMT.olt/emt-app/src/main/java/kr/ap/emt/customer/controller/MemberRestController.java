...
public class MemberRestController extends AbstractController {
	...
	public ResponseEntity<?> changePwdPost() {
		...
		try {
			...
			ApIssueTemporaryPassword issueTemporaryPassword = (ApIssueTemporaryPassword) WebUtils
					.getSessionAttribute(getRequest(), "TEMP_PW_CHANGE");

			CheckResult result2 = apApi
					.requestIssueTemporaryPassword(issueTemporaryPassword);
			if (result2.isResult()) {
				WebUtils.setSessionAttribute(getRequest(), "TEMP_PW_CHANGE",
						issueTemporaryPassword);
			} else {
				throw error(result, HttpStatus.INTERNAL_SERVER_ERROR, "ERROR",
						"비밀번호 전송에 실패했습니다. 아래 재발급 버튼을 눌러 다시 시도해주세요.");
			}
		} catch (Exception e2) {
			throw error(result, HttpStatus.INTERNAL_SERVER_ERROR, "ERROR",
					"비밀번호 전송에 실패했습니다. 아래 재발급 버튼을 눌러 다시 시도해주세요.");
	...
	public ResponseEntity<?> sendId(String phoneNo) {
		...
		CheckResult rsltVo = apApi
				.requestMemberIdGuide(paramApRequestMemberIdGuide);
		if (rsltVo.isResult()) {
			return ResponseEntity.ok("{}");
		}
	...
	public ResponseEntity<?> recoveryUser() {
		String token = (String) SessionUtils.getAttribute(getRequest(),
				SessionKey.TEMP_MEMBER_SN);
		APRequestContext
				.setAccessToken(token.substring(token.indexOf("||") + 2));
	...
	/**
	 * 간편ID 확인. 이름, 생년월일 폰번호로 아이디 찾기.
	 */
	@PostMapping("/find/findId/simple")
	public ResponseEntity<?> findIdSimple(HttpServletRequest req,
			HttpServletResponse resp, MemberDTO user) {
		return findIdSimpleM(resp, user);
	}

	/**
	 * 아이디 찾기 외국인 번호로 아이디를 검색.
	 */
	@PostMapping("/find/findId/foreigner")
	public ResponseEntity<?> findIdForeigner(HttpServletRequest req,
			HttpServletResponse resp, String memberName, String frgrRegNum) {
		return findIdForeignerM(resp, memberName, frgrRegNum);
	}

	/**
	 * 통합API '휴대폰 번호'로 고객 본인인증 받기 SMS전송.
	 *
	 * @param request
	 * @return
	 */
	...
	/**
	 * 헨드폰 번호로 본인 인증. 통합API를 사용하여 본인인증.
	 *
	 * @param request
	 * @return
	 */
	...
	private ResponseEntity<?> findIdSimpleM(HttpServletResponse resp,
		...
		if (StringUtils.isEmpty(user.getMemberName())
				|| StringUtils.isEmpty(user.getAthtDtbr())
				|| StringUtils.isEmpty(user.getPhoneNumber())) {
			throw error(respMap, HttpStatus.INTERNAL_SERVER_ERROR, "INFOERR",
					"누락된 정보가 있습니다.");
		}
		...
		findMember = apApi.findMemberId(user.getMemberName(), athtDt, ","
				+ user.getPhoneNumber());

		if (findMember == null || findMember.getMemberId() == null) {
			throw error(respMap, HttpStatus.INTERNAL_SERVER_ERROR, "EAPI001",
					getMessage("customer.find.notFindId"));
		}
		...
		respMap.put(
				"message",
				getMessage("customer.find.findIdComplete", findMember
						.getMemberId().substring(0, 3) + "***",
						dateFormat.format(findMember.getMemberSignupDt())));
	...
	private ResponseEntity<?> findIdForeignerM(HttpServletResponse resp,
		...
		/*
		 * MemberDTO member = loginService.findUserForeigner(memberName,
		 * frgrRegNum); if(member == null) { respMap.put("state", "regist");
		 * return ResponseEntity.ok(respMap); }
		 */
	...
	private ResponseEntity<?> custSelfOnlineM(HttpServletRequest request) {
		String custNm = request.getParameter("custNm") == null ? "" : request
				.getParameter("custNm");
		String frclCd = request.getParameter("frclCd") == null ? "" : request
				.getParameter("frclCd");
		String sxclCd = request.getParameter("sxclCd") == null ? "" : request
				.getParameter("sxclCd");
		String athtDtbr = request.getParameter("athtDtbr") == null ? ""
				: request.getParameter("athtDtbr");
		String cellNum = request.getParameter("cellNum") == null ? "" : request
				.getParameter("cellNum");
		String phoneCorp = request.getParameter("phoneCorp") == null ? ""
				: request.getParameter("phoneCorp");

		HashMap<String, Object> result = new HashMap<String, Object>();

		if (!phoneCorp.isEmpty() && !custNm.isEmpty() && !frclCd.isEmpty()
				&& !sxclCd.isEmpty() && !athtDtbr.isEmpty()
				&& !cellNum.isEmpty()) {
			...
			try {
				CicuemCuInfTotTcVo cicuemCuInfTotTcVo2 = amoreAPIService
						.certifycustselfonline(cicuemCuInfTotTcVo);

				if ("Y".equals(cicuemCuInfTotTcVo2.getR_result())) {
					CicuemCuInfTotTcVo cicuemCuInfTotTcVo3 = new CicuemCuInfTotTcVo();
					cicuemCuInfTotTcVo3.setR_certNum(cicuemCuInfTotTcVo2
							.getR_certNum());
					cicuemCuInfTotTcVo3.setSmsNum(cicuemCuInfTotTcVo2
							.getSmsNum());
					cicuemCuInfTotTcVo3.setR_check_1(cicuemCuInfTotTcVo2
							.getR_check_1());
					cicuemCuInfTotTcVo3.setR_check_2(cicuemCuInfTotTcVo2
							.getR_check_2());
					cicuemCuInfTotTcVo3.setR_check_3(cicuemCuInfTotTcVo2
							.getR_check_3());
					...
					cicuemCuInfTotTcVo3.setJoinChCd(APConstant.EH_CH_CD);
					cicuemCuInfTotTcVo3.setJoinPrtnId(APConstant.EH_PRTN_ID);

					WebUtils.setSessionAttribute(request,
							SessionKey.KMS_CHECK_VO, cicuemCuInfTotTcVo3);
				} else {
					throw error(result, HttpStatus.INTERNAL_SERVER_ERROR,
							"EAPI001", "인증요청에 실패하였습니다. 입력 정보를 다시 한 번 확인해주세요.");
				}
				...
				throw error(result, HttpStatus.INTERNAL_SERVER_ERROR,
						"unknown", "알 수 없는 오류입니다. 잠시 후 다시 시도해주세요.");
			}
		} else {
			throw error(result, HttpStatus.INTERNAL_SERVER_ERROR, "INFOERR",
					"누락된 정보가 있습니다.");
		}
	...
	private ResponseEntity<?> authorizationPhoneNumberM(// FIXME asdfs
		...
		String certNum = request.getParameter("smsNum") == null ? "" : request
				.getParameter("smsNum");
		String chptchaKey = request.getParameter("captchaKey") == null ? ""
				: request.getParameter("captchaKey");
		String chptcha = request.getParameter("captcha") == null ? "" : request
		...
		if (!certNum.isEmpty() && !chptchaKey.isEmpty() && !chptcha.isEmpty()) {
			...
			if (!check) {
				throw error(result, HttpStatus.INTERNAL_SERVER_ERROR,
						"chptcha", "입력한 숫자가 이미지 숫자와 일치하지 않습니다.");
			}

			CicuemCuInfTotTcVo cicuemCuInfTotTcVo = (CicuemCuInfTotTcVo) WebUtils
					.getSessionAttribute(request, SessionKey.KMS_CHECK_VO);
			...
			if (APConstant.KIST9201.equals(cicuemCuInfTotTcVo2.getR_rsltCd())) {
				throw error(result, HttpStatus.INTERNAL_SERVER_ERROR, "expire",
						"만료된 인증번호입니다. 재인증 버튼을 눌러주세요.");
			}
			if (!APConstant.KIST0000.equals(cicuemCuInfTotTcVo2.getR_rsltCd())) {
				throw error(result, HttpStatus.INTERNAL_SERVER_ERROR,
						"certNum", "인증번호를 잘못 입력하셨습니다.");
			}
			if (APConstant.RESULT_OK.equals(cicuemCuInfTotTcVo2.getRsltCd())) {
				SignupStatusResult signStatus = apApi
						.getSignupStatus(cicuemCuInfTotTcVo2.getCiNo());
				if (!signStatus.isMember()) {
					throw error(result, HttpStatus.INTERNAL_SERVER_ERROR,
							"nonRegist", "미가입");
				}
				ApFindMemberIdResult memberResult = apApi
						.findMemberIdByCI(cicuemCuInfTotTcVo2.getCiNo());
				result.put("id", memberResult.getMemberId());
				result.put(
						"message",
						getMessage("customer.find.findIdComplete",
								memberResult.getMemberId(),
								memberResult.getMemberSignupDt()));
				MemberSession memberSession = getMemberSession();
				memberSession.setUser_incsNo(cicuemCuInfTotTcVo2.getIncsNo());
				setMemberSession(memberSession);

			} else if (APConstant.EXIST_CH_JOIN_USER.equals(cicuemCuInfTotTcVo2
					.getRsltCd())) {

...
import net.g1project.ecp.api.model.ap.ap.ApMemberForPost;
import net.g1project.ecp.api.model.ap.ap.ApMemberPostResult;
import net.g1project.ecp.api.model.ap.ap.ApSimplifiedMemberForPost;
...
public class SignupRestController extends AbstractController {
     ...
     * @return
     */
    @PostMapping("/foreignCert")
    @ResponseBody
    public ResponseEntity<?> foreignCert(@Valid MemberForm memberForm, BindingResult bindingResult) {
		HashMap<String, Object> result = new HashMap<String, Object>();

		memberForm.setValidFlag("foreignCert");
		validate(memberForm, bindingResult, memberFormValidator);
		...
		if (bindingResult.hasErrors()) {
			...
			String custNm = memberForm.getCustNm();
			String frgrRegNum = memberForm.getFrgrRegNum();
			String cellNum = memberForm.getCellNum();

			CicuemCuInfTotTcVo cicuemCuInfTotTcVo = new CicuemCuInfTotTcVo();
			cicuemCuInfTotTcVo.setCustNm(custNm);
			cicuemCuInfTotTcVo.setJoinChCd("004");

			cicuemCuInfTotTcVo.setFrclCd("F");

			//주민등록번호
			cicuemCuInfTotTcVo.setFrgrRegNum1(frgrRegNum.substring(0, 6));
			cicuemCuInfTotTcVo.setFrgrRegNum2(frgrRegNum.substring(6, 12));
			if ("5".equals(frgrRegNum.substring(6, 7))) {
				cicuemCuInfTotTcVo.setSxclCd("M");
			} else if ("6".equals(frgrRegNum.substring(6, 7))) {
				cicuemCuInfTotTcVo.setSxclCd("F");
			}
			...
			try {
				...
				//TODO: ap api - 외국인인증1(등록번호)(certifyforeignonline)
				////고객명	custNm
				////외국인등록번호 앞번호	frgrRegNum1
				////외국인등록번호 뒷번호	frgrRegNum2
				////외국인구분코드	frclCd
				////성별구분코드	sxclCd
				////휴대폰식별전화번호	cellTidn
				////휴대폰국전화번호	cellTexn
				...
				CicuemCuInfTotTcVo cicuemCuInfTotTcVoRslt = amoreAPIService.certifyforeignonline(cicuemCuInfTotTcVo);

				if ("Y".equals(cicuemCuInfTotTcVoRslt.getR_result())) {

					if (!StringUtils.isEmpty(cicuemCuInfTotTcVoRslt.getIncsNo())) {
						//이미 가입된회원
						result.put("incsNo", cicuemCuInfTotTcVoRslt.getIncsNo());
						...
						//가입되지 않은 회원
						result.put("r_certNum", cicuemCuInfTotTcVoRslt.getR_certNum());
					}
    ...
    public ResponseEntity<?> stepThree(@Valid MemberForm memberForm, BindingResult bindingResult) {
		...
		if (bindingResult.hasErrors()) {
			...
			try {
				...
				cicuemCuInfTotTcVo.setChCd(APConstant.EH_CH_CD);
...
//				cicuemCuAdtInfTcVo.setWdanDt("19991231");
				...
				cicuedCuChCsTcVo.setChCd(APConstant.EH_POS_CH_CD);
				...
				cicuedCuChCsTcVo = new CicuedCuChCsTcVo();
				cicuedCuChCsTcVo.setChCd(APConstant.EH_CH_CD);
				cicuedCuChCsTcVo.setChcsNo(chcsNo);
//				cicuedCuChTcVo.setUserPwdEc(userPwdEc);// 03-21 As Is Legacy는 입력 하지 마세요. 경로(030) webID password
				cicuedCuChCsTcVo.setPrtnNm(getPrntNm(cicuedCuChCsTcVo.getChCd()));
				cicuedCuChCsTcVo.setFscrId("AC919532");
				cicuedCuChCsTcVo.setLschId("AC919532");
				list.add(cicuedCuChCsTcVo);
				...
				if(memberForm.getTerms() != null && memberForm.getTerms()) {

					CicuemCuOptiCsTcVo2 = new CicuemCuOptiCsTcVo();
					if(memberForm.getSms()) {
						CicuemCuOptiCsTcVo2.setSmsOptiYn("Y");
						CicuemCuOptiCsTcVo2.setSmsOptiDt(today);
					} else {
						CicuemCuOptiCsTcVo2.setSmsOptiYn("N");
					}
					if(memberForm.getEmail()) {
						CicuemCuOptiCsTcVo2.setEmlOptiYn("Y");
						CicuemCuOptiCsTcVo2.setEmlOptiDt(today);
					} else {
						CicuemCuOptiCsTcVo2.setEmlOptiYn("N");
					}
					
					CicuemCuOptiCsTcVo2.setDmOptiYn("N");
					CicuemCuOptiCsTcVo2.setTmOptiYn("N");
					CicuemCuOptiCsTcVo2.setFscrId("AC919532");
					CicuemCuOptiCsTcVo2.setLschId("AC919532");
					CicuemCuOptiCsTcVo2.setChCd(APConstant.EH_CH_CD);
					list2.add(CicuemCuOptiCsTcVo2);
					
					CicuemCuOptiCsTcVo2 = CicuemCuOptiCsTcVo2.clone();
					CicuemCuOptiCsTcVo2.setChCd(APConstant.EH_POS_CH_CD);
					CicuemCuOptiCsTcVo2.setChNm(getPrntNm(CicuemCuOptiCsTcVo2.getChCd()));
					list2.add(CicuemCuOptiCsTcVo2);
				} else {
					CicuemCuOptiCsTcVo2 = CicuemCuOptiCsTcVo2.clone();
				...
				try {
					logger.info("가입 직전 상태조회 CI : " + cicuemCuInfTotTcVo.getCiNo());
				...
				if (!ObjectUtils.isEmpty(cicuemCuInfCoOutVo)
					&& (APConstant.RESULT_OK.equals(cicuemCuInfCoOutVo.getRsltCd())) || joinChCd ) {
					...
					if(memberForm.getTerms() != null && memberForm.getTerms()) {
						...
						if(memberForm.getSms()) {
							s.setReceiveTypeCode("Email");
							s.setReceiveAgreeCode("Agree");
						} else {
							s.setReceiveTypeCode("Email");
							s.setReceiveAgreeCode("NotAgree");
						}
						s = new SignupReceiveAgree();
						signupReceiveAgrees.add(s);
						if(memberForm.getEmail()) {
							s.setReceiveTypeCode("SMS");
							s.setReceiveAgreeCode("Agree");
						} else {
							s.setReceiveTypeCode("SMS");
							s.setReceiveAgreeCode("NotAgree");
						...
						s = new SignupReceiveAgree();
						s.setReceiveTypeCode("DM");
						s.setReceiveAgreeCode("NotAgree");
						signupReceiveAgrees.add(s);
						
						s = new SignupReceiveAgree();
						s.setReceiveTypeCode("TM");
						s.setReceiveAgreeCode("NotAgree");
						signupReceiveAgrees.add(s);
    ...
    public ResponseEntity<?> simpleJoin(boolean terms, boolean sms, boolean email) {
    	...
    	apMemberPostInfo.setIncsNo(getMemberSession().getUser_incsNo());
    	List<SignupTermsAgree> signupTermsAgree = new ArrayList<SignupTermsAgree>();
    	SignupTermsAgree termsVo = new SignupTermsAgree();
    	termsVo.setTermsDisplayCode("ET001");
    	termsVo.setAgreeYn("Y");
		signupTermsAgree.add(termsVo);
		apMemberPostInfo.setSignupTermsAgree(signupTermsAgree);
		
		List<SignupReceiveAgree> signupReceiveAgree = new ArrayList<SignupReceiveAgree>();
		SignupReceiveAgree smsVo = new SignupReceiveAgree();
		smsVo.setReceiveTypeCode("SMS");
		smsVo.setReceiveAgreeCode(sms? "Agree":"NotAgree");
		signupReceiveAgree.add(smsVo);
		SignupReceiveAgree emailVo = new SignupReceiveAgree();
		emailVo.setReceiveTypeCode("Email");
		emailVo.setReceiveAgreeCode(email? "Agree":"NotAgree");
		signupReceiveAgree.add(emailVo);
		
		apMemberPostInfo.setSignupReceiveAgree(signupReceiveAgree);
		try {
			apApi.simplifiedSignupMember(apMemberPostInfo);

...
;(function ( $, $B ) {
    ...
    var Common = {
      ...
      timeCountDown: function (min, callback) {
		...
		replaceImagePath : function( path, width, height ){
			if ( path ) {
				var split = path.split( '.' );
				split[split.length - 2] += '_' + width + '_' + height;
				path = split.join( '.' );
			}
			return path;
		},
		...
		removeHtmlTag : function(strHtml){
			...
			strHtml = strHtml.replace(/alert/ig, '');
    ...
    /** ========== server load image to resize image ========== */
	( function () {
		...
		for(var i = 0; i < arr.length; i++){
			...
			if( img.hasAttribute('data-width') ){
				...
				img.setAttribute('src', AP.common.replaceImagePath(src, width, height) );
			}
		}
	} )();

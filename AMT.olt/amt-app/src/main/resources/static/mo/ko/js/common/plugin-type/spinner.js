	...
	var Spinner = function ( $target, pluginName ) {
		...
		this.setValue = function ( value ) {
			if ( !_$target.hasClass('disabled') ) {
				...
				_$input.val( $B.string.format(_currentValue, CIPHER) );
				setBtnState();
			}
		};
		...
		this.getValue = function () {
			return Math.floor( parseInt(_$input.val()) );
		};
		...
		function btnClickHandler (e) {
			...
			_$input.val( $B.string.format(getCorrectValue(value), CIPHER) );
			setBtnState();
		...
		function inputChangeHandler (e) {
			if ( e.type === 'keyup' ) {
				if ( isEnterKey(e.which) ) {
					_$input.val( $B.string.format(getCorrectValue(_this.getValue()), CIPHER) );
					setBtnState();
					checkChange( _currentValue );
				}
			} else {

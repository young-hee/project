	...
	var PageTitle = $B.Class.extend({
		...
		_draw: function () {
			if ( this._depth1Index ) {
				var depth1Html = AP.common.getTemplate( 'display.category.page-title', this._data );
				this._$target.find( '.breadcrumb' ).append( depth1Html );
			}
			if ( this._depth2Index ) {
				var depth2Html = AP.common.getTemplate( 'display.category.page-title', this._data.submenus[this._depth1Index - 1] );
				this._$target.find( '.breadcrumb' ).append( depth2Html );
			}
			if ( this._depth3Index ) {
				var depth3Html = AP.common.getTemplate( 'display.category.page-title', this._data.submenus[this._depth1Index - 1].submenus[this._depth2Index - 1] );

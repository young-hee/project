    ...
    var Accordion = function ( $target, pluginName ) {
		...
		this.open = function ( index, silent, init ) {
			if ( index > -1 && index < _$target.find('> dl').length ) {
				...
				_$target.find( '> dl:eq( ' + index + ' ) >dd.reverse' ).attr( 'aria-hidden', true ).hide();

				if ( init ) {
					_$target.find( '> dl:eq( ' + index + ' ) >dd:not(.reverse)' ).attr( 'aria-hidden', false ).stop().show();
				} else {
					if ( _isSingleOpen ) {
						this.close();
						_$target.find( '> dl:eq(' + index + ') >dt' ).addClass( 'on' );
					}
					_$target.find( '> dl:eq( ' + index + ' ) >dd:not(.reverse)' ).attr( 'aria-hidden', false ).stop().slideDown({
						queue: false,
						duration: 300,
						complete: function (e) {
		...
		this.close = function ( index, silent, init ) {
			if ( index > -1 && index < _$target.find('> dl').length ) {
				...
				_$target.find( '> dl:eq( ' + index + ' ) >dd.reverse' ).attr( 'aria-hidden', false ).show();

				if ( init ) {
					_$target.find( '> dl:eq( ' + index + ' ) >dd:not(.reverse)' ).attr( 'aria-hidden', true ).stop().hide();
				} else {
					_$target.find( '> dl:eq( ' + index + ' ) >dd:not(.reverse)' ).attr( 'aria-hidden', true ).stop().slideUp({
						queue: false,
						duration: 300
					});
				}
				...
				_$target.find( '> dl >dd.reverse' ).attr( 'aria-hidden', false ).show();
				_$target.find( '> dl >dd:not(.reverse)' ).attr( 'aria-hidden', true ).stop().slideUp({
					queue: false,
					duration: 300
				});
			}
